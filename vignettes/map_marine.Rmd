...
As we work on fishes, we search for synonyms into FishBase to verify species names from the BOLD dataset.
...
predResBold.verified <- fishbase_verify_bold_species(predResBold)
```


### 3. Transform curated BOLD data.frame into spatialpoints object from sp package

To work with spatial environmental data, we transform our BOLD data.frame into spatialpoints object with the right projection.

```{r}
predResBold.verified.sp <- spatialpoints_bold_df(predResBold.verified, projectionCRS="+init=epsg:3347")
```


## 

First, let’s download world coastlines data from natural earth.

```{r}
grid.sp <- grid_spatialpoints(siteSize=260000,
